...
* `proxy` — Manage the DNS server and TLS-terminating proxy
...
* `-x`, `--exec <EXEC>` — Exec once up with the given command [default: configured defaultExec]
...
* `<CMD>` — command to run [default: Configured defaultExec]
